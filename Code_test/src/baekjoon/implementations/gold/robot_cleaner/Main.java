...
//        System.out.println(y + " " + x + " " + Graph[y][x]);
        ...
        while (true) {
            ...
            if (isClean) {
                for (int i = 1; i < 5; i++) {
                    int tmpD = (i + tmp[2]) % 4;
                    int nextX = tmp[1] + dx[tmpD];
                    int nextY = tmp[0] + dy[tmpD];
                    if (nextY >= 0 && nextX >= 0 && nextY < Graph.length && nextX < Graph[0].length) {
                        if (!cleaned[nextY][nextX] && Graph[nextY][nextX] == 0) {
                            q.offer(new int[]{nextY, nextX, tmpD});
                            break;
                        }
                    }
                }
...
//                }
                ...
                int nextX = tmp[1] + dx[(2 + tmp[2]) % 4];
                int nextY = tmp[0] + dy[(2 + tmp[2]) % 4];
                q.offer(new int[]{nextY, nextX, tmp[2]});
                if (nextY >= 0 && nextX >= 0 && nextY < Graph.length && nextX < Graph[0].length) {

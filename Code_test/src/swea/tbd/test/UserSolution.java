...
public class UserSolution {
    ...
    void addPlayer(Player np, int l, Player p) {
        while (np != null && (np.ability > p.ability || (np.ability == p.ability
            && np.id < p.id))) {
            np = np.next;
        }
    ...
    int move() {
        ...
        for (int i = 0; i < L - 1; i++) {
            if (pMids[i].ability < hs[i].ability || (pMids[i].ability == hs[i].ability
                && pMids[i].id > hs[i].id)) {
            ...
            if (pMids[i + 1].ability < ls[i].ability || (pMids[i + 1].ability == ls[i].ability
                && pMids[i + 1].id > ls[i].id)) {
    ...
    int trade() {
        ...
        for (int i = 0; i < L - 1; i++) {
            ...
            if (pMids[i + 1].ability < ms[i].ability || (pMids[i + 1].ability == ms[i].ability
                && pMids[i + 1].id > ms[i].id)) {
